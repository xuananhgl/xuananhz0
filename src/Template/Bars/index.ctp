    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th scope="col"><?= $this->Paginator->sort('ENTREPRISES') ?></th>
                <th scope="col"><?= $this->Paginator->sort('SECTEUR') ?></th>
                <th scope="col"><?= $this->Paginator->sort('PROPOS') ?></th>
...
<th scope="col"><?= $this->Paginator->sort('links') ?></th>
        ...
        <tbody>
            ...
            <tr>
                <td><?= $this->Number->format($bar->id_bar) ?></td>
                <td><?= h($bar->ENTREPRISES ) ?></td>
                <td><?= h($bar->SECTEUR) ?></td>
                <td><?= h($bar->PROPOS) ?></td>
                <td><?= h($bar->CONTACT) ?></td>
                <td><?= h($bar->BLOG) ?></td>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $bar->id_bar]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $bar->id_bar]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $bar->id_bar], ['confirm' => __('Are you sure you want to delete # {0}?', $bar->id_bar)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
